...
public:
    ...
    Node* cloneGraph(Node* node) {
        //deep clone - Cloned graph has all node as new nodes but don't have 
        // node as a reference to original graph
        if(node ==NULL){
            return NULL;
        }

        vector<Node*> visited(1000,NULL);   //making a node* visited array to store the new nodes

        Node* copy= new Node(node->val);

        //iterate to all neighbours
        for(auto curr: node->neighbors){

            if(visited[curr->val]== NULL){  //not visited node
